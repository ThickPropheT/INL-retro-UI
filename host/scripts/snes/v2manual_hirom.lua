...
local function rom_manf_id( debug )
	...
	elseif (manf_id == 0x01 and prod_id == 0x7E) then
		...
		return true
	elseif (manf_id == 0xC2 and prod_id == 0xCB) then
		print("MXIC 4-8MB flash detected")
		return true
	else
		return false
	end
...
local function erase_sector( sect_num, debug )

	local rv = nil

	print("Erasing TSSOP flash sector:", sect_num)

	--WR $AAA:AA $555:55 $AAA:AA
	--dict.snes("SNES_SET_BANK", 0x00)
	--8MByte flash has 64KByte sectors aside from top/bottom sector split into 8x8KByte sectors
	--so normally A16 is the LSbit of the sector number
	dict.snes("SNES_SET_BANK", (sect_num + 0xC0)) --erasing HIROM banks start at bank $C0
	...
	dict.snes("SNES_ROM_WR", 0x8AAA, 0xAA)
	dict.snes("SNES_ROM_WR", 0x8555, 0x55)
	dict.snes("SNES_ROM_WR", 0x8AAA, 0x30) --0x30 sector erase written to sector address
--	A15 hi/lo based on odd/even bank
--	if sect_num % 2 then
--		--odd banks
--		dict.snes("SNES_ROM_WR", 0x8AAA, 0x30) --0x30 sector erase written to sector address
--	else	--even banks
--		dict.snes("SNES_ROM_WR", 0x0AAA, 0x30) --0x30 sector erase written to sector address
--	end
...
local function process(process_opts, console_opts)
	...
	dict.pinport("CTL_OP", "IRQ")
	dict.pinport("CTL_SET_LO", "IRQ")
...
--test cart by reading manf/prod ID
	if test then
		...
		print("snes mapping:", snes_mapping)
...
--		local bank = 0
--		while wr_cnt >0 do
--			dict.snes("SNES_ROM_WR", 0x8000, bank)
...
--			print("SNES bank:", bank, "reset vector", string.format("$%x", vector) )
--			wr_cnt = wr_cnt -1
--		end
--
--		wr_cnt = 20
...
--erase the cart
	if process_opts["erase"] then
		...
		erase_flash()
	--	512KB / 64KB sectors = 4 sectors
	--	erase_sector(0) --64KByte per sector
	--	erase_sector(1) --128KByte
	--	erase_sector(2)
	--	erase_sector(3) --256KByte
	--	erase_sector(4)
	--	erase_sector(5)
	--	erase_sector(6)
	--	erase_sector(7) --512KByte
...
--program flashfile to the cart
	if process_opts["program"] then
		...
		file = assert(io.open(process_opts["flash_filename"], "rb"))
...
--verify flashfile is on the cart
	if process_opts["verify"] then
		...
		if (files.compare( process_opts["verify_filename"], process_opts["flash_filename"], true ) ) then
